 ...
 * Nếu chỉ sửa trade mà để nguyên `allocationAmount` thì Σ allocationAmount ≠
 * netAmount, và Strategy Allocation ở §16 sẽ sai vĩnh viễn.
 *
 * ĐỔI CHIẾN LƯỢC LÀ ĐƯỜNG SỬA LỖI NHẬP, KHÔNG PHẢI ĐỔI Ý.
 *
 * Chiến lược là QUAN SÁT từ giao dịch (§4): một lệnh vào vì luận điểm nào thì mang
 * luận điểm đó. Chọn nhầm ô lúc nhập là chuyện có thật, và nếu không sửa được thì
 * cách duy nhất còn lại là huỷ lệnh rồi nhập lại — làm hỏng cả trình tự thời gian
 * mà giá vốn bình quân phụ thuộc vào.
 *
 * Bỏ trống toàn bộ ô phân bổ = GIỮ NGUYÊN bộ chiến lược cũ. Đây là mặc định an
 * toàn: người chỉ muốn sửa một con số không vô tình xoá mất phân bổ.
 */
export async function updateTradeAction(_prev: unknown, formData: FormData): Promise<ActionResult> {
  ...
  try {
    ...
    /*
     * PHÂN BỔ MỚI — rỗng nghĩa là giữ nguyên, không phải xoá hết.
     *
     * `readAllocations` bỏ qua mọi ô trống, nên một form chỉ sửa khối lượng sẽ trả về
     * mảng rỗng. Hiểu mảng rỗng thành "xoá sạch phân bổ" sẽ để lại một lệnh không
     * thuộc chiến lược nào — tổng vị thế vẫn đúng còn phần theo chiến lược thì hụt,
     * đúng kiểu sai âm thầm mà §16 khổ sở nhất.
     */
    const phanBoMoi = readAllocations(formData);
    const doiChienLuoc = phanBoMoi.length > 0;

    if (doiChienLuoc) {
      const tongBps = phanBoMoi.reduce((x, y) => x + y.allocationBps, 0);
      if (tongBps !== BPS_TOTAL) {
        return {
          ok: false,
          fieldErrors: {
            allocations: [
              `Tổng tỷ lệ phân bổ phải đúng 100%, đang là ${(tongBps / 100).toFixed(2)}%.`,
            ],
          },
        };
      }

      const idMoi = phanBoMoi.map((x) => x.strategyId);
      if (new Set(idMoi).size !== idMoi.length) {
        return {
          ok: false,
          fieldErrors: { allocations: ['Một chiến lược chỉ được khai một lần.'] },
        };
      }

      /*
       * CHIẾN LƯỢC PHẢI CÓ THẬT VÀ ĐANG BẬT — hỏi database, không tin form.
       *
       * `strategyId` là chuỗi trong `FormData`. Không kiểm thì một id bịa sẽ bị Prisma
       * từ chối bằng lỗi khoá ngoại bằng tiếng Anh, còn một id CÓ THẬT nhưng đã tắt
       * thì lọt hẳn — và lệnh gắn vào một chiến lược không còn ai dùng.
       */
      const hopLe = await prisma.strategy.findMany({
        where: { id: { in: idMoi }, isActive: true },
        select: { id: true },
      });
      if (hopLe.length !== idMoi.length) {
        return {
          ok: false,
          fieldErrors: {
            allocations: ['Có chiến lược không tồn tại hoặc đã ngừng dùng. Chọn lại.'],
          },
        };
      }
    }
    ...
    /*
     * Tỷ lệ lấy từ phân bổ MỚI nếu có, ngược lại giữ tỷ lệ cũ. Số tiền thì LUÔN tính
     * lại — đây là điểm dễ bỏ sót nhất khi sửa lệnh.
     */
    const bpsList = doiChienLuoc
      ? phanBoMoi.map((x) => x.allocationBps)
      : before.strategies.map((s) => s.allocationBps);
    ...
    const after = await prisma.$transaction(async (tx) => {
      ...
      if (doiChienLuoc) {
        /*
         * XOÁ RỒI GHI LẠI, không cố khớp từng dòng cũ với dòng mới.
         *
         * Bộ chiến lược mới có thể nhiều hơn, ít hơn, hoặc khác hẳn bộ cũ. Ghép cặp
         * theo vị trí là cách chắc chắn gán nhầm tiền của chiến lược này sang chiến
         * lược kia. Cả hai thao tác nằm trong CÙNG một transaction nên không có khoảnh
         * khắc nào lệnh này không thuộc chiến lược nào.
         */
        await tx.tradeStrategy.deleteMany({ where: { tradeId } });
        await tx.tradeStrategy.createMany({
          data: phanBoMoi.map((x, i) => ({
            tradeId,
            strategyId: x.strategyId,
            allocationBps: x.allocationBps,
            allocationAmount: newAmounts[i]!,
          })),
        });
      } else {
      ...
      await writeAudit({
        ...
        before: {
          ...
          fees: before.fees,
          tax: before.tax,
          phanBo: before.strategies.map((x) => `${x.strategyId}:${x.allocationBps}`).join(),
          netAmount: netAmount(
            before.transactionType as 'BUY' | 'SELL',
            before.quantity,
            before.price,
        ...
        after: {
          ...
          phanBo: (doiChienLuoc
            ...
            .map((x) => `${x.strategyId}:${x.allocationBps}`)
            .join(),
          netAmount: newNet,
        ...
        note:
          `Lý do: ${reason}. ` +
          (doiChienLuoc
            ? 'Phân bổ chiến lược được thay bằng bộ mới.'
            : 'Phân bổ chiến lược giữ nguyên tỷ lệ, tính lại số tiền.'),
        ...meta,
      });
...
// ---------------------------------------------------------------------------
// Xoá hẳn giao dịch (§20)
// ---------------------------------------------------------------------------

/**
 * XOÁ HẲN MỘT GIAO DỊCH KHỎI DATABASE.
 *
 * VÌ SAO CÓ, TRONG KHI ĐÃ CÓ HUỶ. Huỷ để lại một dòng mang nhãn "đã huỷ" — đúng cho
 * một lệnh có thật nhưng không thực hiện nữa. Nó KHÔNG đúng cho một dòng chưa bao giờ
 * tồn tại ngoài đời: thành viên gõ nhầm mã, nhầm tài khoản, bấm hai lần. Giữ lại một
 * dòng như vậy nghĩa là bảng giao dịch không còn khớp với sao kê của sàn, và mỗi lần
 * đối chiếu lại phải giải thích một dòng không có thật.
 *
 * KHÔNG HOÀN TÁC ĐƯỢC. Dòng biến mất cùng toàn bộ `trade_strategies` và tệp đính kèm
 * (khoá ngoại `onDelete: Cascade`). Bù lại:
 *
 *   - BẮT BUỘC NÊU LÝ DO, như sửa và huỷ.
 *   - NHẬT KÝ CHỤP TOÀN BỘ dòng trước khi xoá, kể cả phân bổ chiến lược. Sau khi xoá,
 *     bản ghi đó là thứ DUY NHẤT còn lại — nên nó phải đủ để dựng lại lệnh bằng tay,
 *     chứ không phải chỉ ghi "đã xoá một lệnh".
 *
 * CÙNG CHỐT VỊ THẾ ÂM NHƯ HUỶ. Xoá một lệnh MUA đã khớp mà phần đó đã bị bán đi sẽ
 * làm vị thế âm — một trạng thái không tồn tại ngoài đời. Chặn trước, và nói rõ phải
 * xử lý lệnh bán nào.
 */
export async function deleteTradeAction(
  ...
  const actor = await requirePermission('transaction.delete');
  ...
  try {
    const trade = await prisma.trade.findUniqueOrThrow({
      ...
      include: {
        stock: { select: { symbol: true } },
        strategies: { select: { strategyId: true, allocationBps: true, allocationAmount: true } },
        brokerAccount: { select: { broker: true, accountNo: true } },
    ...
    if (
      ...
      trade.transactionType === TRANSACTION_TYPE.BUY
    ) {
      const held = await heldQuantity(trade.portfolioId, trade.stockId);
      if (held - trade.quantity < 0) {
        return {
          ok: false,
          message:
            `Không xoá được: sau khi xoá, vị thế ${trade.stock.symbol} sẽ âm ` +
            `${(held - trade.quantity).toLocaleString('vi-VN')}. Xử lý các lệnh bán liên quan trước.`,
        };
      }
    }
    ...
    await prisma.$transaction(async (tx) => {
      /*
       * GHI NHẬT KÝ TRƯỚC KHI XOÁ, trong cùng transaction.
       *
       * Ghi sau thì có một khoảnh khắc dòng đã mất mà chưa có gì ghi lại; nếu bước ghi
       * hỏng ở đó, lệnh biến mất không dấu vết. Cùng transaction nên hoặc cả hai cùng
       * thành công, hoặc không có gì xảy ra.
       */
      await writeAudit({
        tx,
        actor,
        ...
        before: {
          ...
          nguoiThucHien: trade.executor.fullName,
          taiKhoan: trade.brokerAccount
            ? `${trade.brokerAccount.broker} ${trade.brokerAccount.accountNo}`
            : null,
          isStockDividend: trade.isStockDividend,
          phanBo: trade.strategies.map(
            (x) => `${x.strategyId}:${x.allocationBps}:${x.allocationAmount}`,
          ),
        },
    ...
    return {
      ok: true,
      message: `Đã xoá ${trade.code} (${trade.transactionType} ${trade.stock.symbol}). Không hoàn tác được — xem Nhật ký nếu cần dựng lại.`,
    };
  } catch (error) {
    return { ok: false, message: error instanceof Error ? error.message : 'Xoá giao dịch thất bại.' };
  }
}
