 ...
 *
 * MANG THEO SÀN vì ô chọn phải lọc theo sàn của tài khoản đang khai: một IB làm
 * việc cho đúng một công ty chứng khoán, nên IB của VPS không được hiện khi người
 * dùng đang khai tài khoản SSI. Lọc ở đây là để dễ dùng — chốt thật nằm ở server
 * (`kiemIb` trong accounts/actions.ts), vì ô chọn của trình duyệt gửi tay được.
 */
...
function tenSan(v: string | null | undefined): string {
  return (v ?? '').trim().toLowerCase().replace(/\s+/g, ' ');
}

/**
 * IB dùng được cho một tài khoản ở sàn này.
 *
 * Với sàn "Khác" phải so CẢ TÊN công ty: hai công ty chứng khoán khác nhau cùng rơi
 * vào mã OTHER, nên chỉ so mã sẽ gom chúng làm một.
 */
export function ibCungSan(
  ibs: IbOption[],
  broker: Broker,
  brokerOther: string | null | undefined,
): IbOption[] {
  return ibs.filter(
    (x) =>
      x.broker === broker &&
      (broker !== BROKER.OTHER || tenSan(x.brokerOther) === tenSan(brokerOther)),
  );
}
...
export function AddBrokerAccountForm({
  ...
  const [tenSanKhac, setTenSanKhac] = useState('');
  const [open, setOpen] = useState(false);

  /*
   * Ô chọn IB chỉ hiện IB CÙNG SÀN với sàn đang chọn ở ô trên.
   *
   * Đổ cả danh mục ra rồi để server từ chối là bắt người dùng đoán: họ thấy một IB
   * trong danh sách, chọn nó, rồi bị báo lỗi. Danh sách chỉ nên chứa thứ chọn được.
   */
  const ibChonDuoc = ibCungSan(ibs, broker, tenSanKhac);
        ...
        {broker === BROKER.OTHER ? (
          <label className="block">
            ...
            <input
              name="brokerOther"
              value={tenSanKhac}
              onChange={(e) => setTenSanKhac(e.target.value)}
              className={inputCls}
        ...
        */}
        <label className="block">
          ...
          {ibs.length === 0 ? (
            ...
            /*
              DANH MỤC CÓ IB NHƯNG KHÔNG IB NÀO Ở SÀN NÀY — nói thẳng ra.

              Nếu chỉ để ô chọn trống, người dùng sẽ đọc thành "hệ thống hỏng" hoặc
              "mình chưa được cấp quyền", rồi khai đại là mở trực tiếp — một câu trả
              lời SAI nằm lẫn với những câu đúng trên biểu đồ vốn theo IB.
            */
            <span className="mt-1 block text-tiny text-warn-500">
              Danh mục chưa có IB nào làm việc tại{' '}
              {broker === BROKER.OTHER ? (tenSanKhac || 'công ty này') : BROKER_LABEL_VI[broker]}.
              Nhờ quản trị khai IB đó ở Cơ cấu tổ chức, hoặc chọn “Mở trực tiếp”.
            </span>
          ) : null}
        </label>
...
/**
 * ĐỔI IB CỦA MỘT TÀI KHOẢN.
 *
 * Ô chọn chỉ liệt kê IB CÙNG SÀN với tài khoản — IB của VPS không xuất hiện trên một
 * tài khoản mở tại SSI. Cùng lý do như ô chọn ở form khai tài khoản: danh sách chỉ
 * nên chứa thứ chọn được, chứ không phải để server từ chối sau khi người dùng đã chọn.
 *
 * "Mở trực tiếp" NẰM TRONG DANH SÁCH, vì rời khỏi IB cũng là một cách chuyển: IB nghỉ
 * mà chưa có ai thay thì tài khoản về trạng thái không qua IB, không phải kẹt lại ở
 * tên một người không còn làm việc đó nữa.
 */
export function ChangeIbForm({
  accountId,
  accountLabel,
  currentIbId,
  ibs,
  ...
  /** Toàn bộ IB đang bật; component tự lọc theo sàn của tài khoản này. */
  ibs: IbOption[];
  broker: Broker;
  brokerOther: string | null;
  /**
   * Người đang bấm KHÔNG phải chủ tài khoản, mà là người quản lý danh mục IB.
   *
   * Chỉ đổi chữ trên nút và câu nhắc — chốt quyền nằm ở server. Nhưng nói ra thì
   * người đó biết mình đang sửa tài khoản của người khác, thay vì tưởng là của mình.
   */
  boiQuanTri?: boolean;
}) {
  const [mo, setMo] = useState(false);
  const [state, action, pending] = useActionState(changeAccountIbAction, EMPTY);

  const chonDuoc = ibCungSan(ibs, broker, brokerOther).filter((x) => x.id !== currentIbId);
  ...
  if (!mo) {
    return (
      <button
        ...
        title={
          boiQuanTri
            ? `Chuyển ${accountLabel} sang IB khác cùng sàn (tài khoản của người khác)`
            : `Chuyển ${accountLabel} sang IB khác cùng sàn`
        }
  ...
  return (
    <form action={action} className="w-full rounded-lg border border-ink-700 bg-ink-850 p-3 text-left">
      <input type="hidden" name="id" value={accountId} />

      <p className="mb-2 text-tiny text-slate-muted">
        Chuyển IB · <span className="font-mono text-slate-soft">{accountLabel}</span>
        {boiQuanTri ? <span className="ml-1 text-warn-500">(tài khoản của người khác)</span> : null}
      </p>

      <select name="ibId" defaultValue="" className={inputCls} required>
      ...
      {chonDuoc.length === 0 ? (
        <p className="mt-1 text-tiny text-warn-500">
          Không có IB nào khác ở cùng công ty chứng khoán với tài khoản này.
          {currentIbId !== null ? ' Chỉ chuyển về “mở trực tiếp” được.' : ''}
        </p>
      ) : null}

      {/*
        NÓI TRƯỚC HỆ QUẢ, KHÔNG ĐỂ PHÁT HIỆN TRÊN BIỂU ĐỒ.

        Biểu đồ "Tài khoản & vốn theo IB" gom vốn theo IB HIỆN TẠI của tài khoản, nên
        chuyển xong thì toàn bộ vốn đã nạp trong quá khứ hiện dưới IB mới. Đó là lựa
        chọn có chủ đích, nhưng người bấm cần biết trước — nếu không, lần đầu nhìn
        Dashboard họ sẽ tưởng số liệu hỏng.
      */}
      <p className="mt-2 text-tiny text-ink-500">
        Vốn đã nạp trước đây sẽ tính cho IB mới trên biểu đồ vốn theo IB. Phí và thuế
        của lệnh cũ không đổi — chúng đã được ghi vào từng lệnh lúc nhập.
      </p>
      ...
      <KetQuaDoiIb state={state} />
    </form>
  );
}

function KetQuaDoiIb({ state }: { state: ActionResult }) {
  if (!state.message) return null;
  return (
    <p className={`mt-2 text-tiny ${state.ok ? 'text-up-500' : 'text-down-500'}`} role="status">
      {state.message}
    </p>
  );
}
