  ...
  return (
    <div className="flex min-h-screen">
      ...
      <aside className="hidden w-60 shrink-0 flex-col border-r border-ink-800 bg-ink-900 lg:flex">
        ...
        {/*
          THẺ NGƯỜI DÙNG ĐÃ CHUYỂN LÊN GÓC PHẢI THANH TRÊN.

          Trước đây nó nằm ở chân sidebar, và hệ quả là nó BIẾN MẤT ở mọi khổ màn
          hình hẹp hơn `lg` — đúng lúc người dùng cần biết mình đang đăng nhập bằng
          tài khoản nào nhất. Phải dựng một bản sao riêng trên thanh trên để bù,
          tức là hai chỗ cùng nói một điều và sớm muộn lệch nhau.

          Một chỗ duy nhất, luôn nhìn thấy, và tự thu lại còn chữ viết tắt khi hẹp.
        */}
      </aside>
        ...
        <header className="flex items-center justify-between gap-2 border-b border-ink-800 bg-ink-900/60 px-3 py-2 backdrop-blur sm:gap-4 sm:px-5 sm:py-3">
          ...
          */}
          <div className="flex min-w-0 items-center gap-1.5 lg:hidden">
            ...
            {/*
              CHỮ NHẬN DIỆN ẨN DƯỚI 480px, và có `truncate` để không bao giờ tràn.

              Mốc cũ là 375px, đo cho cụm bên phải của bản trước. Đo lại ở khổ 375 sau
              khi thẻ người dùng chuyển lên đây: cụm phải rộng 284px, cộng nút mở menu
              và lề là 367px — chữ nhận diện không còn chỗ, và nó KHÔNG co lại mà tràn
              đè lên cụm phải (đo được: chữ chiếm 62→154, cụm phải bắt đầu ở 79).

              Hai lớp bảo vệ, vì mốc breakpoint là một con số đo được hôm nay: `truncate`
              khiến chữ tự cắt thay vì đè lên thứ khác ở bất kỳ khổ nào chưa lường tới.
              Trong những thứ đang chen nhau ở đây, chữ nhận diện vẫn là thứ duy nhất
              KHÔNG làm được việc gì — nên nó là thứ bị bỏ trước.
            */}
            <p className="hidden min-w-0 truncate text-tiny font-semibold tracking-[0.16em] text-accent-400 min-[480px]:block">
              {BRAND_NAME_UPPER}
            </p>
          </div>
          ...
          <div className="ml-auto flex items-center gap-4">
            ...
            {/*
              NGƯỜI DÙNG NẰM NGOÀI CÙNG BÊN PHẢI.

              Đây là vị trí quy ước của danh tính trên gần như mọi phần mềm có thanh
              trên; đặt nó đúng chỗ người ta đưa mắt tới nghĩa là không phải đi tìm.

              THU VỀ CHỮ VIẾT TẮT KHI HẸP. Chữ viết tắt vẫn phân biệt được hai tài
              khoản khác nhau — thứ duy nhất cần trả lời ở đây là "tôi đang là ai".
              Tên đầy đủ và vai trò quay lại từ `lg` trở lên, nơi có chỗ thật.

              Vai trò nằm ở dòng thứ hai của thẻ này, nên nhãn `RoleBadge` cũ đã bỏ:
              hai chỗ in cùng một chữ chỉ tốn diện tích của thanh trên.
            */}
            <Link
              href="/profile"
              title={`${user.fullName} · ${user.roleNameVi ?? user.email}`}
              className="flex items-center gap-2 rounded-lg py-1 pr-1 pl-1 transition hover:bg-ink-800 lg:pr-2"
            >
              <span className="relative grid size-8 shrink-0 place-items-center rounded-full bg-accent-600/20 text-xs font-semibold text-accent-400">
              ...
              <span className="hidden max-w-40 text-left lg:block">
                <span className="block truncate text-xs font-medium text-strong">
                  {user.fullName}
                </span>
                <span className="block truncate text-tiny text-slate-muted">
