...
export const introducingBrokerSchema = z
  .object({
    code: orgCode,
    name: orgName,
    /*
     * SÀN CỦA IB — dùng đúng bộ mã với `brokerAccountSchema`.
     *
     * Hai bên PHẢI so được với nhau: chốt "IB cùng sàn với tài khoản" là một phép
     * so bằng giữa hai trường này. Nếu IB khai sàn bằng chữ tự do thì phép so đó
     * thành so chuỗi người gõ, và "SSI" với "ssi " sẽ là hai sàn khác nhau.
     */
    broker: z
      .string()
  ...
  .refine((d) => d.broker !== BROKER.OTHER || Boolean(d.brokerOther), {
    message: 'Chọn “Khác” thì phải điền tên công ty chứng khoán',
    path: ['brokerOther'],
  });
...
export const dividendSchema = z.object({
  ...
  occurredAt: z.coerce.date(),
  /*
   * KHỐI LƯỢNG ĐƯỢC HƯỞNG CỔ TỨC — không phải số đang giữ.
   *
   * Cổ tức trả theo số cổ phiếu nắm tại NGÀY CHỐT QUYỀN. Hai con số lệch nhau ngay khi
   * có một lệnh mua hoặc bán giữa ngày chốt và hôm nay, và bên lệch nguy hiểm là bán:
   * bán xong thì số đang giữ nhỏ hơn số được hưởng, tính theo số đang giữ là ghi thiếu
   * tiền vào sổ mà không có gì báo.
   *
   * Mức trần nằm ở action chứ không ở đây: zod không biết tài khoản đang giữ bao nhiêu.
   */
  eligibleQuantity: z
    .string()
    .trim()
    .transform((v) => v.replace(/[.,\s]/g, ''))
    .refine((v) => /^\d+$/.test(v), { message: 'Khối lượng phải là số nguyên dương' })
    .transform((v) => Number(v))
    .refine((v) => v > 0, { message: 'Khối lượng phải lớn hơn 0' })
    .refine((v) => Number.isSafeInteger(v), { message: 'Khối lượng quá lớn' }),
