...
export function evaluatePortfolioRules(
  ...
  for (const rule of rules) {
    ...
    if (metric === RISK_METRIC.CASH_BPS) {
      ...
      /*
       * ĐO PHẦN GIẢI NGÂN ĐƯỢC, KHÔNG ĐO TIỀN MẶT CÒN.
       *
       * Quy tắc này nói "không còn dư địa giải ngân", nên thứ nó phải đo là phần được
       * phép dùng sau khi giữ quỹ dự phòng. Khi `availableCash` đổi nghĩa thành tiền mặt
       * còn, để nguyên ở đây sẽ làm cảnh báo âm thầm KHÓ NỔ HƠN trước — đúng loại thay
       * đổi tệ nhất: một chốt an toàn bị nới mà không ai chủ ý nới nó.
       */
      const measured = BigInt(
        ratioToBps(summary.cash.deployableCash, summary.portfolioValue),
      );
      if (!compareThreshold(measured, comparator, rule.threshold)) continue;
      ...
      findings.push({
        ...
        context: {
          scope,
          metric,
          deployableCash: summary.cash.deployableCash.toString(),
