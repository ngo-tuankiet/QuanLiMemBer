...
export const PERMISSIONS: readonly PermissionDef[] = [
   ...
   * cũng đọc danh sách mã.
   */
  p('stock', 'view', 'View stock master', 'Xem danh mục mã chứng khoán'),

  /*
   * `stock.create` ĐÃ QUAY LẠI — update / delete thì không.
   *
   * Bỏ nó đi từng là quyết định đúng khi mọi mã cần dùng đều đã nằm trong
   * `STOCK_SEED`. Nhưng đường "sửa seed rồi chạy lại db:seed" đòi người dùng phải
   * sửa mã nguồn để làm một việc hằng ngày: muốn theo dõi một mã ngoài 117 mã có
   * sẵn (SCS chẳng hạn) thì không lấy được giá, và cũng không nhập được lệnh.
   * Một cổng quyền đóng chặt tới mức người dùng phải mở editor không phải là cổng
   * quyền — nó là việc thiếu.
   *
   * KHÔNG KÉO THEO update / delete. Thêm một mã là việc cộng thêm, không đụng dữ
   * liệu nào đang có. Sửa hay xoá một mã ĐÃ CÓ GIAO DỊCH thì khác hẳn: nó viết lại
   * ý nghĩa của những lệnh đã khớp trong quá khứ. Hai việc đó cần bàn riêng, không
   * đi kèm miễn phí theo quyền này.
   *
   * §7 VẪN NGUYÊN VẸN: cấm là cấm DỮ LIỆU NGOÀI tự tạo mã (cổng nạp đẩy mã lạ vào
   * `unknownSymbols`). Người có quyền khai một mã kèm ngành và phân ngành là con
   * đường ngược lại — có chủ ý, có nhật ký kiểm toán, và có đủ thông tin mà luồng
   * giá của VNStock không hề mang theo.
   */
  p('stock', 'create', 'Add stock to master data', 'Thêm mã vào danh mục chuẩn'),
