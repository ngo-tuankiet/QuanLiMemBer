...
export interface CashBreakdown {
  ...
  /** Tiền mặt còn trong các tài khoản. KHÔNG trừ quỹ dự phòng — xem `computeCash`. */
  availableCash: bigint;
  /** Phần được phép giải ngân: tiền mặt còn TRỪ quỹ dự phòng. Không âm. */
...
export interface PortfolioSummary {
  ...
  allocation: {
    investedBps: number;
    /**
     * TOÀN BỘ SỐ DƯ TIỀN — khả dụng CỘNG quỹ dự phòng.
     *
     * Biểu đồ phân bổ hiển thị tiền thành MỘT phần, theo quyết định của người dùng.
     * Tính thẳng từ `cashBalance` chứ không cộng `cashBps + reserveBps`: hai tỷ lệ
     * đó đã làm tròn riêng, cộng lại có thể lệch 1 bps so với chính con số tiền —
     * đủ để ba phần không còn cộng lại bằng 100%.
     */
    cashBps: number;
    /**
     * Phần quỹ dự phòng nằm TRONG `cashBps`, không phải phần thứ ba.
     *
     * Vẫn tính và vẫn trả về: biểu đồ gộp hai phần lại, nhưng quỹ dự phòng vẫn là
     * một chính sách có thật — nó vẫn trừ vào "Tiền khả dụng" ở Dashboard, và trang
     * phân bổ vẫn cần nói ra phần bị giữ lại là bao nhiêu.
     */
    reserveBps: number;
  };
...
export async function computeCash(
  ...
  return {
    ...
    reserveShortfall,
    /*
     * TIỀN KHẢ DỤNG = TIỀN MẶT CÒN. Không trừ quỹ dự phòng.
     *
     * Bản trước trừ, và nó sai theo hai cách. Sai về nghĩa: người dùng đọc "tiền khả
     * dụng" là "trong tài khoản còn bao nhiêu tiền", và đó là câu họ cần khi đối chiếu
     * với sao kê của sàn. Sai về tính nhất quán: trang thành viên vốn đã tính
     * `Σ số dư các tài khoản` và KHÔNG trừ gì — nên cùng một nhãn mang hai nghĩa ở hai
     * trang, và con số lệch nhau đúng bằng mức dự phòng.
     *
     * Quỹ dự phòng KHÔNG mất đi: nó vẫn được tính và vẫn hiện trên thẻ, kèm phần còn
     * dùng được sau khi giữ lại (`deployableCash`). Chỗ nào cần "bao nhiêu được phép
     * giải ngân" thì đọc con số đó, đúng tên của nó.
     */
    availableCash: cashBalance,
    /*
     * Phần được phép giải ngân sau khi giữ lại quỹ dự phòng.
     *
     * Đây là con số mà `availableCash` từng mang, nay có tên đúng việc. Không âm: mức
     * dự phòng đã bị kẹp theo số tiền đang có, nên khi tiền ít hơn dự phòng thì nó
     * bằng 0 chứ không âm.
     */
    deployableCash: cashBalance - reserveAmount,
    scope,
...
export async function computePortfolioSummary(
  ...
  return {
    ...
    allocation: {
      ...
      cashBps: ratioToBps(cash.cashBalance, portfolioValue),
...
export async function computeTeamPerformance(
  ...
  options: {
    onlyTeamId?: string;
    /**
     * BỘ LỌC CHIẾN LƯỢC / NGÀNH của trang đang gọi.
     *
     * Thiếu tham số này, khối "Vốn & lãi/lỗ theo nhóm" đứng yên trong khi mọi ô
     * khác trên Dashboard đổi theo bộ lọc — người đọc thấy hai con số mâu thuẫn
     * trên cùng một màn hình và không có gì nói cái nào đang trả lời câu hỏi nào.
     *
     * KHÔNG NHẬN `teamId` ở đây: nhóm đã có `onlyTeamId`, và để hai đường cùng đặt
     * một thứ là mở cửa cho chúng nói ngược nhau.
     */
    filter?: Pick<EngineFilter, 'strategyId' | 'sectorId' | 'from' | 'to'>;
  } = {},
): Promise<TeamPerformance[]> {
  const loc = options.filter ?? {};
  ...
  for (const team of selected) {
    ...
    const [positions, tradeCount] = await Promise.all([
      computePositions({ ...loc, portfolioId, teamId: team.teamId }),
      /*
       * SỐ LỆNH DÙNG CHUNG `tradeWhere` với phần tính tiền.
       *
       * Bản trước đếm bằng một `where` viết tay chỉ có danh mục và nhóm. Khi bộ lọc
       * chiến lược bật lên, cột "Lệnh" vẫn là con số của cả nhóm trong khi cột tiền
       * bên cạnh đã thu hẹp — hai cột cùng một hàng nói về hai tập lệnh khác nhau.
       */
      prisma.trade.count({
        where: tradeWhere({ ...loc, portfolioId, teamId: team.teamId }),
      }),
    ]);
    ...
    const netCapital = trades.reduce((sum, t) => {
      /*
       * LỌC THEO CHIẾN LƯỢC THÌ CHIA TIỀN, KHÔNG LẤY CẢ LỆNH.
       *
       * `computePositions` ở trên chỉ tính phần vốn thuộc chiến lược đang lọc. Nếu ở
       * đây lấy nguyên số tiền của lệnh thì "vốn triển khai" lớn hơn hẳn "giá vốn" của
       * cùng một hàng — mà hai con số đó người ta đọc cạnh nhau để suy ra lãi/lỗ.
       */
      const shareBps = loc.strategyId
        ? (t.strategies as { allocationBps: number }[]).reduce(
            (x, a) => x + a.allocationBps,
            0,
          )
        : BPS_TOTAL;
      if (shareBps === 0) return sum;

      const full = netAmount(
      ...
      const net = divRound(full * BigInt(shareBps), BigInt(BPS_TOTAL));
...
// ---------------------------------------------------------------------------
// Vị thế nằm ở tài khoản nào (§9)
// ---------------------------------------------------------------------------

/** Một phần vị thế của một mã, nằm ở một tài khoản chứng khoán cụ thể. */
export interface HoldingByAccount {
  /** `null` = lệnh chưa gắn tài khoản nào (dữ liệu có trước khi trường này bắt buộc). */
  brokerAccountId: string | null;
  /** Mã sàn; chuỗi rỗng khi lệnh chưa gắn tài khoản. */
  broker: string;
  brokerOther: string | null;
...
/**
 * VỊ THẾ CỦA TỪNG MÃ ĐANG NẰM Ở NHỮNG TÀI KHOẢN NÀO.
 *
 * Bảng vị thế trả lời "giữ bao nhiêu", không trả lời "giữ ở đâu" — mà đó là câu phải
 * biết trước khi đặt lệnh bán: lệnh bán đi theo một tài khoản cụ thể, và bán 4.200 CP
 * từ một tài khoản chỉ có 1.000 CP là lệnh không đặt được ngoài đời.
 *
 * DÙNG ĐÚNG `tradeWhere` VÀ ĐÚNG PHÉP CHIA THEO CHIẾN LƯỢC của `computePositions`.
 * Đây là điều kiện bắt buộc, không phải chi tiết cài đặt: hai hàm mà chọn lệnh khác
 * nhau thì Σ theo tài khoản sẽ lệch cột "Khối lượng" ngay bên cạnh nó, và người đọc
 * không có cách nào biết con số nào đúng. Vì vậy phép chia micro ở đây sao y bản
 * chính — kể cả bước làm tròn cuối.
 *
 * Trả về Map theo `stockId`; mỗi mã một danh sách đã bỏ phần ≤ 0 và xếp nhiều nhất
 * trước.
 */
export async function computeHoldingsByAccount(
  filter: EngineFilter,
): Promise<Map<string, HoldingByAccount[]>> {
  const trades = await prisma.trade.findMany({
    where: tradeWhere(filter),
    select: {
      stockId: true,
      ...
      transactionType: true,
      quantity: true,
      brokerAccount: { select: { broker: true, brokerOther: true, accountNo: true } },
      strategies: filter.strategyId
        ? { where: { strategyId: filter.strategyId }, select: { allocationBps: true } }
        : false,
    },
  ...
  interface Gop extends HoldingByAccount {
    quantityMicro: bigint;
  }

  // Khoá gộp: mã + tài khoản. `__none__` cho lệnh chưa gắn tài khoản — một nhóm
  // riêng, không trộn vào tài khoản nào.
  const theoMa = new Map<string, Map<string, Gop>>();

  for (const t of trades) {
    const shareBps = filter.strategyId
      ? (t.strategies as { allocationBps: number }[]).reduce((s, a) => s + a.allocationBps, 0)
      : BPS_TOTAL;
    if (shareBps === 0) continue;

    let theoTk = theoMa.get(t.stockId);
    if (!theoTk) {
      theoTk = new Map();
      theoMa.set(t.stockId, theoTk);
    }

    const khoa = t.brokerAccountId ?? '__none__';
    let g = theoTk.get(khoa);
    if (!g) {
      g = {
        brokerAccountId: t.brokerAccountId,
        broker: t.brokerAccount?.broker ?? '',
        brokerOther: t.brokerAccount?.brokerOther ?? null,
        accountNo: t.brokerAccount?.accountNo ?? null,
        quantity: 0,
        quantityMicro: 0n,
      };
      theoTk.set(khoa, g);
    ...
    const qtyMicro = divRound(BigInt(t.quantity) * MICRO * BigInt(shareBps), BigInt(BPS_TOTAL));
    g.quantityMicro += t.transactionType === TRANSACTION_TYPE.BUY ? qtyMicro : -qtyMicro;
  }

  const out = new Map<string, HoldingByAccount[]>();
  for (const [stockId, theoTk] of theoMa) {
    const ds = [...theoTk.values()]
      .map((g) => ({
        brokerAccountId: g.brokerAccountId,
        broker: g.broker,
        brokerOther: g.brokerOther,
        accountNo: g.accountNo,
        quantity: Number(divRound(g.quantityMicro, MICRO)),
      }))
      /*
       * BỎ PHẦN ≤ 0. Một tài khoản đã bán hết mã đó thì không còn "đang nằm ở đây"
       * nữa; giữ lại dòng 0 chỉ làm danh sách dài ra bằng những chỗ không có gì.
       * Số âm thì càng phải bỏ: nó là dấu hiệu bán nhiều hơn mua TRONG PHẠM VI đang
       * lọc (bán ở tài khoản này phần đã mua ở tài khoản khác), và cộng nó vào sẽ
       * làm tổng theo tài khoản nhỏ hơn cột Khối lượng.
       */
      .filter((x) => x.quantity > 0)
      .sort((a, b) => b.quantity - a.quantity);

    if (ds.length > 0) out.set(stockId, ds);
  }
