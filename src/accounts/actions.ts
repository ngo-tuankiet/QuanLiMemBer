 ...
 *
 * IB PHẢI CÙNG SÀN VỚI TÀI KHOẢN. Một IB làm việc cho đúng một công ty chứng khoán
 * (`introducing_brokers.broker`); tài khoản mở tại SSI không thể do IB của VPS quản
 * lý. Ô chọn trên trang đã lọc theo sàn, nhưng lọc ở trình duyệt không phải là chốt —
 * chốt nằm ở đây.
 *
 * VỚI SÀN "KHÁC" THÌ SO CẢ TÊN. Hai công ty chứng khoán khác nhau cùng rơi vào mã
 * `OTHER`, nên chỉ so mã sẽ cho một IB của công ty A quản lý tài khoản ở công ty B.
 * So thêm tên (bỏ qua hoa/thường và khoảng trắng thừa) là thứ duy nhất phân biệt được
 * hai trường hợp đó.
 *
...
async function kiemIb(
  ibId: string | undefined,
  sanCuaTaiKhoan: string,
  tenSanCuaTaiKhoan: string | null | undefined,
): Promise<ActionResult | null> {
  if (!ibId) return null;

  const ib = await prisma.introducingBroker.findUnique({
    where: { id: ibId },
    select: { isActive: true, code: true, broker: true, brokerOther: true },
  });
  ...
  if (ib.broker !== sanCuaTaiKhoan) {
    return {
      ok: false,
      fieldErrors: {
        ibId: [
          `IB ${ib.code} làm việc tại ${brokerLabel(ib.broker, ib.brokerOther)}, ` +
            `không phải ${brokerLabel(sanCuaTaiKhoan, tenSanCuaTaiKhoan ?? null)}. ` +
            'Chỉ chọn được IB cùng công ty chứng khoán với tài khoản.',
        ],
      },
    };
  }
  ...
  if (
    ib.broker === BROKER.OTHER &&
    chuanHoaTenSan(ib.brokerOther) !== chuanHoaTenSan(tenSanCuaTaiKhoan)
  ) {
    return {
      ok: false,
      fieldErrors: {
        ibId: [
          `IB ${ib.code} làm việc tại "${ib.brokerOther}", không phải ` +
            `"${tenSanCuaTaiKhoan ?? ''}". Chỉ chọn được IB cùng công ty chứng khoán.`,
        ],
      },
    };
  }
...
export async function createBrokerAccountAction(
  ...
  const ibHong = await kiemIb(input.ibId, input.broker, input.brokerOther);
...
// ---------------------------------------------------------------------------
// Chuyển tài khoản sang IB khác (cùng sàn)
// ---------------------------------------------------------------------------

/**
 * ĐỔI IB CỦA MỘT TÀI KHOẢN — chuyển qua lại giữa các IB trong CÙNG một công ty
 * chứng khoán.
 *
 * VÌ SAO LÀ MỘT ACTION RIÊNG, KHÔNG DÙNG `updateBrokerAccountAction`. Cái kia sửa cả
 * sàn lẫn số tài khoản, nên nó cần nguyên form và nó phải là việc của chính chủ. Đổi
 * IB thì khác về bản chất: nó không đụng vào danh tính tài khoản, chỉ đổi người giới
 * thiệu đang quản lý. Ghép hai việc vào một action sẽ buộc quản trị viên phải được
 * quyền sửa cả số tài khoản của người khác để làm được một việc nhỏ hơn hẳn.
 *
 * HAI NGƯỜI ĐƯỢC LÀM, VÌ NÓ XẢY RA TỪ HAI PHÍA:
 *
 *   chính chủ        tự biết tài khoản mình chuyển sang ai phụ trách.
 *   người có ib.manage   IB nghỉ việc thì toàn bộ tài khoản phải sang IB khác, và
 *                        chờ từng chủ tài khoản tự sửa là không xong.
 *
 * Đây là NGOẠI LỆ ĐẦU TIÊN của quy tắc "tài khoản chứng khoán chỉ chính chủ đụng
 * vào", và nó hẹp đúng bằng một trường: `ibId`. Số tài khoản, sàn, trạng thái đóng/mở
 * vẫn chỉ chính chủ sửa được.
 *
 * KHÔNG CÓ LỊCH SỬ IB. Vốn đã nạp trước khi chuyển sẽ hiện dưới IB MỚI trên biểu đồ
 * "Tài khoản & vốn theo IB", vì `computeIbExposure` gom theo `ibId` hiện tại. Đây là
 * lựa chọn có chủ đích của người dùng: biểu đồ trả lời "giờ ai đang quản lý", không
 * phải "ai đã mang vốn về". Phí và thuế của lệnh CŨ không bị ảnh hưởng — chúng được
 * tính lúc nhập lệnh rồi ghi thẳng vào dòng lệnh.
 */
export async function changeAccountIbAction(
  _prev: unknown,
  formData: FormData,
): Promise<ActionResult> {
  const actor = await requireUser();
  const id = String(formData.get('id') ?? '');
  const ibIdMoi = String(formData.get('ibId') ?? '') || undefined;
  ...
  const laChinhChu = account.userId === actor.id;
  const quanTri = actor.permissions.has('ib.manage');
  if (!laChinhChu && !quanTri) {
    return {
      ok: false,
      message: 'Chỉ chủ tài khoản hoặc người quản lý danh mục IB được đổi IB của tài khoản này.',
    };
  }

  if ((account.ibId ?? '') === (ibIdMoi ?? '')) {
    return { ok: false, message: 'IB không thay đổi.' };
  }

  // Cùng chốt với lúc khai tài khoản: có thật, đang bật, và CÙNG SÀN.
  const ibHong = await kiemIb(ibIdMoi, account.broker, account.brokerOther);
  if (ibHong) return ibHong;

  const ibMoi = ibIdMoi
    ? await prisma.introducingBroker.findUnique({
        where: { id: ibIdMoi },
  ...
  const after = await prisma.brokerAccount.update({
    where: { id },
    data: { ibId: ibIdMoi ?? null },
    select: { id: true, ibId: true },
  });

  const ten = (x: { code: string; name: string } | null): string =>
    x ? `${x.code} · ${x.name}` : 'Mở trực tiếp (không qua IB)';
  ...
  await writeAudit({
    ...
    entityId: id,
    entityLabel: accountLabel(account.broker, account.brokerOther, account.accountNo),
    before: { ibId: account.ibId, ib: ten(account.ib) },
    after: { ibId: after.ibId, ib: ten(ibMoi) },
    /*
     * Ghi rõ người làm là ai ĐỐI VỚI tài khoản này. "Quản trị đổi IB tài khoản của
     * người khác" và "chủ tài khoản tự đổi" là hai việc cần phân biệt được khi đọc
     * lại nhật ký, mà chỉ nhìn tên người thao tác thì không suy ra được.
     */
    note: laChinhChu
      ? 'Chủ tài khoản tự đổi IB.'
      : `Người quản lý danh mục IB đổi IB cho tài khoản của ${account.user.fullName}.`,
    ...(await requestMeta()),
  });
  ...
  return {
    ok: true,
    message: `Đã chuyển ${account.accountNo} từ ${ten(account.ib)} sang ${ten(ibMoi)}.`,
  };
}

// ---------------------------------------------------------------------------
// Sửa / xoá một dòng nạp rút (§20)
// ---------------------------------------------------------------------------

/**
 * Đọc một dòng nạp/rút và kiểm những điều kiện chung của cả sửa lẫn xoá.
 *
 * CHỈ NẠP VÀ RÚT. `capital_flows` còn chứa cổ tức, lãi tiền gửi, thu/chi khác — mỗi
 * loại có đường ghi riêng và ý nghĩa riêng. Cho một màn hình "sửa nạp rút" đụng vào
 * dòng cổ tức nghĩa là sửa được số tiền doanh nghiệp đã trả, mà nó lại không đi kèm
 * phần điều chỉnh vị thế tương ứng.
 */
async function docDongVon(id: string) {
  const flow = await prisma.capitalFlow.findUnique({
    ...
    select: {
      ...
      brokerAccountId: true,
      brokerAccount: {
        select: { userId: true, broker: true, brokerOther: true, accountNo: true },
      },
    },
  });
  ...
  if (
    ...
    flow.flowType !== CAPITAL_FLOW_TYPE.WITHDRAWAL
  ) {
    return {
      loi: `Chỉ sửa được dòng nạp và rút. Dòng này là ${flow.flowType} — sửa ở đúng chỗ đã ghi nó.` as const,
      flow: null,
    };
  }

  return { loi: null, flow };
}

/**
 * SỬA MỘT DÒNG NẠP / RÚT — số tiền, ngày, ghi chú.
 *
 * VÌ SAO CẦN. Thành viên tự khai nạp rút của mình, và gõ thừa một số 0 là chuyện xảy
 * ra. Không có đường sửa thì cách duy nhất là ghi một dòng đối ứng — sổ có hai dòng
 * sai nhau, và tổng thì đúng nhưng lịch sử thì không.
 *
 * KHÔNG ĐỔI ĐƯỢC LOẠI (nạp ↔ rút) và không đổi được tài khoản. Đổi loại là đảo dấu
 * cả khoản tiền: một lần bấm nhầm làm sai gấp đôi số tiền thay vì sai một lần. Nhầm
 * loại thì xoá dòng rồi ghi lại — hai thao tác, và thao tác thứ hai đi qua đúng bộ
 * kiểm của đường ghi bình thường (số dư, chốt bốn mắt với lệnh rút).
 *
 * AI ĐƯỢC SỬA: người có `capital.update`. Không mở cho chính chủ, vì đây là đường đi
 * vòng qua chốt duyệt rút vốn — sửa một khoản rút đã duyệt thành số lớn hơn là rút
 * thêm tiền mà không ai duyệt.
 */
export async function updateCapitalFlowAction(
  _prev: unknown,
  formData: FormData,
): Promise<ActionResult> {
  ...
  const rawDate = String(formData.get('occurredAt') ?? '').trim();
  const note = String(formData.get('note') ?? '').trim();

  if (!reason) {
    return { ok: false, fieldErrors: { reason: ['Bắt buộc nêu lý do sửa.'] } };
  }
  if (!/^\d+$/.test(rawAmount) || rawAmount === '0') {
    return { ok: false, fieldErrors: { amount: ['Số tiền phải là số nguyên dương.'] } };
  }
  const ngay = new Date(rawDate);
  if (Number.isNaN(ngay.getTime())) {
    return { ok: false, fieldErrors: { occurredAt: ['Ngày không hợp lệ.'] } };
  }

  const { loi, flow } = await docDongVon(id);
  if (loi || !flow) return { ok: false, message: loi ?? 'Không tìm thấy dòng vốn.' };

  const amount = BigInt(rawAmount);
  if (amount === flow.amount && ngay.getTime() === flow.occurredAt.getTime() && note === (flow.note ?? '')) {
  ...
  try {
    ...
    const sau = await prisma.capitalFlow.update({
      where: { id },
      data: { amount, occurredAt: ngay, note: note || null },
    ...
    await writeAudit({
      ...
      entityType: ENTITY_TYPE.CAPITAL_FLOW,
      entityId: id,
      entityLabel: nhanDongVon(flow),
      before: { amount: flow.amount, occurredAt: flow.occurredAt, note: flow.note },
      after: { amount: sau.amount, occurredAt: sau.occurredAt, note: note || null },
      note: `Lý do: ${reason}`,
      ...meta,
    });
    ...
    return {
      ok: true,
      message: `Đã sửa ${nhanDongVon(flow)}: ${formatVnd(flow.amount)} → ${formatVnd(amount)}.`,
    };
  } catch (error) {
    return { ok: false, message: error instanceof Error ? error.message : 'Sửa dòng vốn thất bại.' };
  }
}
 ...
 *
 * Cùng lập luận với xoá giao dịch: một khoản chưa bao giờ tồn tại ngoài đời thì giữ
 * lại chỉ làm sổ không khớp sao kê. Không hoàn tác được, nên bắt buộc nêu lý do và
 * nhật ký chụp đủ toàn bộ dòng để dựng lại bằng tay.
 */
export async function deleteCapitalFlowAction(
  _prev: unknown,
  formData: FormData,
): Promise<ActionResult> {
  const actor = await requirePermission('capital.delete');
  ...
  if (!reason) {
    return {
      ok: false,
      fieldErrors: { reason: ['Bắt buộc nêu lý do xoá — đây là bản ghi duy nhất còn lại.'] },
    };
  }
  ...
  try {
    ...
    await prisma.$transaction(async (tx) => {
      // Ghi nhật ký TRƯỚC khi xoá, cùng transaction — xem chú thích ở deleteTradeAction.
      await writeAudit({
        tx,
        actor,
        ...
        before: {
          ...
          note: flow.note,
          teamId: flow.teamId,
          brokerAccountId: flow.brokerAccountId,
        },
        note: `Xoá hẳn khỏi database. Lý do: ${reason}`,
        ...meta,
      });
    ...
    return {
      ok: true,
      message: `Đã xoá ${nhanDongVon(flow)} ${formatVnd(flow.amount)}. Không hoàn tác được.`,
    };
  } catch (error) {
...
/** Nhãn đọc được của một dòng vốn, dùng chung cho nhật ký và thông báo. */
function nhanDongVon(flow: {
  flowType: string;
  brokerAccount: { broker: string; brokerOther: string | null; accountNo: string } | null;
}): string {
  const loai = flow.flowType === CAPITAL_FLOW_TYPE.CONTRIBUTION ? 'nạp vốn' : 'rút vốn';
  if (!flow.brokerAccount) return loai;
  return `${loai} ${accountLabel(
    flow.brokerAccount.broker,
    flow.brokerAccount.brokerOther,
    flow.brokerAccount.accountNo,
  )}`;
