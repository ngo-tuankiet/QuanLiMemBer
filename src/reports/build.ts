  ...
  return {
    ...
    rows: summary.positions
      ...
      .map((pos) => [
        ...
        /*
         * CỔ TỨC TÁCH RIÊNG, ĐÚNG NHƯ TRÊN MÀN HÌNH.
         *
         * `return_pct` là lãi/lỗ GIÁ — chênh lệch trên giá vốn. `total_return_pct`
         * cộng thêm cổ tức tiền mặt. Thiếu ba cột này, người đọc CSV lấy `return_pct`
         * làm "sinh lời" và ra con số nhỏ hơn bảng vị thế trên màn hình cho cùng một
         * mã — hai nguồn số liệu nói khác nhau mà không có gì giải thích.
         *
         * Cổ tức bằng CỔ PHIẾU không nằm ở đây: nó đã vào `quantity` và làm giảm
         * `avg_cost_vnd`. Cộng thêm lần nữa là tính hai lần.
         */
        pos.dividendCash,
        pos.totalReturn,
        csvPercent(pos.totalReturnBps),
 ...
 * Mã sàn xuất ra CSV.
 *
 * Với sàn "Khác" thì lấy TÊN công ty người dùng tự điền — xuất chuỗi `OTHER` cho cả
 * chục công ty khác nhau là gộp chúng làm một ngay trong tệp lưu trữ.
 */
function csvBroker(
  a: { broker: string; brokerOther: string | null } | null | undefined,
): string {
  if (!a) return '';
  return a.broker === BROKER.OTHER ? (a.brokerOther ?? 'OTHER') : a.broker;
}
...
async function buildTransactions(p: ReportParams): Promise<ReportData> {
  const trades = await prisma.trade.findMany({
    ...
    include: {
      ...
      /*
       * TÀI KHOẢN CHỨNG KHOÁN — thứ duy nhất đối chiếu được với sao kê của sàn.
       *
       * Thiếu nó, một người cầm CSV này và một bản sao kê SSI không có cột chung nào
       * để ghép. `null` với lệnh cũ chưa gắn tài khoản, và cột để trống nói đúng điều
       * đó chứ không đoán bừa.
       */
      brokerAccount: {
        select: {
          broker: true,
  ...
  return {
    ...
    rows: trades.map((t) => {
      ...
      return [
        ...
        csvBroker(t.brokerAccount),
        t.brokerAccount?.accountNo ?? '',
        t.brokerAccount?.ib?.code ?? '',
...
async function buildCapitalFlows(p: ReportParams): Promise<ReportData> {
  ...
  return {
    ...
    rows: flows.map((f) => [
      ...
      f.status,
      csvBroker(f.brokerAccount),
      f.brokerAccount?.accountNo ?? '',
      f.team?.nameVi ?? '',
      /*
       * MÃ CHỨNG KHOÁN chỉ có ở dòng cổ tức — cổ tức là một dòng vốn gắn mã. Để trống
       * ở các loại khác là đúng: nạp và rút không thuộc mã nào.
       */
      f.stock?.symbol ?? '',
      f.reference ?? '',
