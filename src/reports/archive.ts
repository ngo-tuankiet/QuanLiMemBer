 ...
 */
/**
 * XUẤT RA để bài kiểm đọc thẳng từng sheet master data.
 *
 * `buildArchive` trả về một Buffer .xlsx — muốn kiểm "ô phí trống chứ không phải 0"
 * thì phải giải nén và phân tích XML, tức là bài kiểm đo bộ giải mã của chính nó chứ
 * không đo dữ liệu. Đọc thẳng dữ liệu trước khi đóng gói là đo đúng thứ cần đo.
 */
export async function masterDataSheets(): Promise<RawSheet[]> {
  const [
    stocks,
    sectors,
    strategies,
    ...
    await Promise.all([
      ...
      /*
       * IB — BIỂU PHÍ VÀ THUẾ NẰM Ở ĐÂY, KHÔNG Ở CHỖ NÀO KHÁC.
       *
       * Sheet Giao dịch xuất ra một con số phí, ví dụ 134.700 ₫. Mở tệp sau một năm
       * mà không có sheet này thì không tra được nó tính theo biểu nào — mà mỗi IB một
       * biểu khác nhau, và `null` (theo mức chung) khác hẳn 0 (miễn phí).
       */
      ...
      /*
       * TÀI KHOẢN CHỨNG KHOÁN — khoá để ghép với sao kê của sàn.
       *
       * Sheet Giao dịch và Dòng vốn nay đều có cột `broker` + `account_no`. Không có
       * sheet này thì hai cột đó là chuỗi trơ: không biết tài khoản thuộc về ai, thuộc
       * IB nào, còn mở hay đã đóng.
       */
      prisma.brokerAccount.findMany({
        orderBy: [{ broker: 'asc' }, { accountNo: 'asc' }],
        select: {
          broker: true,
          brokerOther: true,
          accountNo: true,
  ...
  return [
    ...
    },
    {
      name: 'MD IB',
      headers: [
        'code', 'name', 'broker', 'buy_fee_pct', 'sell_fee_pct', 'sell_tax_pct',
        'is_active', 'note',
      ],
      rows: ibs.map((x) => [
        x.code,
        x.name,
        x.broker === BROKER.OTHER ? (x.brokerOther ?? 'OTHER') : x.broker,
        /*
         * Ô TRỐNG NGHĨA LÀ "THEO MỨC CHUNG", KHÁC HẲN 0 ("miễn phí").
         *
         * Xuất `null` thành 0 sẽ biến mọi IB chưa khai riêng thành IB miễn phí ngay
         * trong tệp lưu trữ — và đó là tệp người ta dùng để đối chiếu về sau.
         */
        x.buyFeeRateBps === null ? '' : (x.buyFeeRateBps / 100).toFixed(2),
        x.sellFeeRateBps === null ? '' : (x.sellFeeRateBps / 100).toFixed(2),
        x.sellTaxRateBps === null ? '' : (x.sellTaxRateBps / 100).toFixed(2),
        x.isActive ? 'YES' : 'NO',
        x.note ?? '',
      ]),
    },
    {
      name: 'MD Tài khoản chứng khoán',
      headers: [
        'broker', 'account_no', 'owner_email', 'owner_name', 'ib_code', 'is_active', 'note',
      ],
      rows: brokerAccounts.map((a) => [
        a.broker === BROKER.OTHER ? (a.brokerOther ?? 'OTHER') : a.broker,
        a.accountNo,
        a.user.email,
        a.user.fullName,
        a.ib?.code ?? '',
