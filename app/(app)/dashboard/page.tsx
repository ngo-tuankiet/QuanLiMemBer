...
export default async function DashboardPage({
  ...
  const [perf, recentTrades, cho, pendingUsers, alerts, teamPerf, memberPerf, ibExposure] =
    await Promise.all([
      ...
      /*
       * KHỐI "Vốn & lãi/lỗ theo nhóm" ĐI THEO BỘ LỌC như mọi khối khác.
       *
       * Trước đây nó chỉ nhận `portfolio.id`, nên đứng yên khi người dùng đổi Nhóm,
       * Chiến lược hay Ngành — trong khi các ô KPI ngay trên nó đã đổi. Hai con số mâu
       * thuẫn trên cùng một màn hình, và không có gì nói cái nào trả lời câu hỏi nào.
       *
       * KHÔNG truyền `from`/`to`: bộ lọc Thời gian trên trang này chỉ dùng cho chuỗi
        ...
        <Kpi
          ...
          sub={
            !cashByTeam
              ? [
                  /*
                   * PHẦN TRĂM TÍNH TỪ CHÍNH CON SỐ THẺ ĐANG HIỆN, không lấy
                   * `allocation.cashBps`.
                   *
                   * `allocation.cashBps` là tỷ trọng của TOÀN BỘ số dư tiền — nó phục vụ
                   * biểu đồ phân bổ, nơi tiền là một lát duy nhất. Thẻ này hiện tiền KHẢ
                   * DỤNG. Dùng lẫn thì có lúc thẻ ghi "₫0 · 14.84% danh mục": số không
                   * đồng mà lại chiếm 14,84% — hai con số trong một thẻ nói về hai thứ
                   * khác nhau, và người đọc kết luận thẻ bị lỗi.
                   */
                  t.kpi.ofPortfolio(
                    formatBps(
                      ratioToBps(whole.cash.availableCash, whole.portfolioValue),
                      false,
                    ),
                  ),
                  ...
                  whole.cash.reserveAmount > 0n
                    ...
                    : null,
                  /*
                   * PHẦN ĐƯỢC PHÉP GIẢI NGÂN — con số mà ô này từng hiện.
                   *
                   * Ô lớn giờ là tiền mặt còn. Nhưng "được phép dùng bao nhiêu" vẫn là câu
                   * phải trả lời, nếu không thì quỹ dự phòng chỉ còn là một dòng chữ không
                   * dẫn tới quyết định nào. Chỉ hiện khi có giữ lại.
                   */
                  whole.cash.reserveAmount > 0n
                    ? t.kpi.deployable(formatCompactVnd_vi(whole.cash.deployableCash))
                    : null,
                  /*
                   * CÒN THIẾU SO VỚI CHÍNH SÁCH — chỉ hiện khi thật sự thiếu.
                   *
                   * Engine đã tính `reserveShortfall` từ lâu nhưng không nơi nào hiện nó.
                   * Đúng lúc tiền khả dụng bằng 0 thì đây là dòng duy nhất giải thích vì
                   * sao: chính sách đòi giữ 480 triệu, tài khoản chỉ còn 440,95 triệu.
                   */
                  whole.cash.reserveShortfall > 0n
                    ? t.kpi.reserveShortfall(
                        formatCompactVnd_vi(whole.cash.reserveShortfall),
                      )
                    : null,
                ]
                  .filter(Boolean)
