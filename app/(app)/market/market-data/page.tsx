...
import { FetchQuoteForm } from './FetchQuoteForm';
 ...
 *
 * Giá đến từ Market Data Service bằng Python gọi `vnstock`. Nhập giá thủ công ĐÃ BỊ
 * BỎ: mã thiếu giá thì bấm lấy giá cho đúng mã đó, không gõ số vào. Những dòng
 * `source = MANUAL` còn lại là dữ liệu cũ từ trước khi có service — bảng bên dưới
 * vẫn đánh dấu chúng, vì biết một con số do ai ghi vào vẫn là việc cần biết.
 */
export default async function MarketDataPage() {
  const user = await requirePagePermission('market_data.view');
  const { t } = await getDict();

  const [status, quotes, quoteTotal, manualTotal, syncs, missingCount, stocksWithoutQuote, nganh] =
    await Promise.all([
    getMarketDataStatus(),
    /*
    ...
    */
    ...
    /*
     * Ngành kèm phân ngành cho ô "Thêm mã". Chỉ lấy ngành CÓ phân ngành đang bật:
     * chọn được một ngành rồi thấy ô phân ngành rỗng là ngõ cụt, mà người dùng không
     * có cách nào tự thoát vì cả hai ô đều bắt buộc.
     */
    prisma.sector.findMany({
      where: { isActive: true, industries: { some: { isActive: true } } },
  ...
  const canAddStock = user.permissions.has('stock.create');
      ...
      */}
      ...
      <div className="mb-5 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        */}
        ...
        <Card className="p-4">
          ...
          <p className="tabular mt-1.5 text-sm font-medium text-strong">
            {quoteTotal - manualTotal} tự động
            {manualTotal > 0 ? ` · ${manualTotal} nhập tay (dữ liệu cũ)` : ''}
          </p>
          <p className="mt-1 text-tiny text-ink-500">
      ...
      {stocksWithoutQuote.length > 0 ? (
        <Card className="mb-5 border-warn-500/30 bg-warn-500/5 p-4">
          ...
          <p className="mt-1 text-xs text-slate-muted">
            ...
            thiếu. Bấm “Cập nhật giá” ở đầu trang, hoặc lấy riêng từng mã ở ô bên dưới.
          </p>
      ...
      {canSync ? (
        <Card className="mb-5 p-5">
          <h2 className="text-sm font-semibold text-strong">Lấy giá theo mã</h2>
          <p className="mt-0.5 mb-4 text-xs text-slate-muted">
            Gọi VNStock cho ĐÚNG một mã, không đụng các mã khác — dùng khi một mã ở danh
            sách trên chưa có giá. Mã phải có sẵn trong danh mục chuẩn (§7): ô này không
            tạo mã mới. Mỗi lần gọi đều có dòng {t.nav.auditLog} và một dòng nhật ký đồng bộ.
          </p>
          <FetchQuoteForm />
        </Card>
      ) : null}
      ...
      {/*
        THÊM MÃ đặt NGAY DƯỚI ô lấy giá, vì đó là chỗ người dùng đụng tường: gõ một mã
        ngoài danh mục thì bị §7 từ chối, và câu trả lời cho "vậy giờ làm sao" phải ở
        ngay dưới câu từ chối. Để nó ở một trang quản trị khác thì người dùng chỉ thấy
        lời từ chối, không thấy lối đi.
      */}
      {canAddStock && nganh.length > 0 ? (
        <Card className="mb-5 p-5">
          <h2 className="text-sm font-semibold text-strong">Thêm mã vào danh mục chuẩn</h2>
          <p className="mt-0.5 mb-4 text-xs text-slate-muted">
            Dùng khi mã bạn cần chưa có trong hệ thống. Ngành và phân ngành là bắt buộc vì
            luồng giá của VNStock không mang theo thông tin đó — để trống thì mã này biến
            mất khỏi {t.dash.sectorExposure} mà không có cảnh báo nào.
          </p>
          <AddStockForm
            nganh={nganh.map((n) => ({
              id: n.id,
              nameVi: n.nameVi,
              phanNganh: n.industries,
            }))}
          />
          <p className="mt-3 text-tiny text-ink-500">
            Mã mới chưa có giao dịch sẽ KHÔNG được lấy giá theo lịch (lịch chỉ chạy cho mã
            đang nắm giữ và rổ VN100). Tới khi có lệnh đầu tiên, dùng ô “Lấy giá theo mã”
            ở trên mỗi khi cần giá mới.
          </p>
        </Card>
      ) : null}
