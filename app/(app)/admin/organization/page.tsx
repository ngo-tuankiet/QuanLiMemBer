...
export default async function OrganizationPage() {
  ...
  const ibRows: IbRow[] = ibs.map((x) => ({
    ...
    name: x.name,
    note: x.note,
    broker: x.broker as Broker,
    brokerOther: x.brokerOther,
              ...
              <li key={x.id} className="flex flex-wrap items-start justify-between gap-3 px-5 py-3">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-baseline gap-2">
                    ...
                    {/*
                      SÀN HIỆN NGAY CẠNH TÊN, không nằm trong ghi chú.

                      Nó không phải thông tin phụ: hai IB trùng tên ở hai công ty chứng
                      khoán khác nhau là chuyện có thật, và sàn là thứ duy nhất phân biệt
                      được — cũng là thứ quyết định IB này nhận được tài khoản nào.
                    */}
                    <span className="rounded border border-accent-500/30 px-1.5 py-px text-micro text-accent-400">
                      {x.broker === BROKER.OTHER ? (x.brokerOther ?? 'Khác') : BROKER_LABEL_VI[x.broker]}
                    </span>
                    {!x.isActive ? (
                      <span className="rounded border border-ink-700 px-1.5 py-px text-micro text-slate-muted">
                        ngừng dùng
