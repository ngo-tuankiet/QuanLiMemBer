...
import { HoldingAccounts } from '@/components/HoldingAccounts';
...
export default async function MemberDetailPage({
  ...
  const [positions, theoTaiKhoan, trades, strategyAllocation, accountBalances] =
    await Promise.all([
    portfolio ? computePositions(filter) : Promise.resolve([]),
    /*
     * CÙNG `filter` VỚI `computePositions`, không phải một bộ lọc na ná.
     *
     * Hai lượt tính đứng cạnh nhau trên cùng một dòng bảng: cột Khối lượng và cột
     * Tài khoản. Lọc khác nhau thì Σ theo tài khoản lệch cột bên cạnh, và không ai
     * biết con số nào mới đúng.
     */
    portfolio ? computeHoldingsByAccount(filter) : Promise.resolve(new Map()),
              ...
              <tbody>
                {open.map((p) => (
                  <tr key={p.stockId} className="border-b border-ink-800 last:border-0">
                    ...
                    {/*
                      TÀI KHOẢN NẰM NGAY SAU KHỐI LƯỢNG.

                      "Giữ bao nhiêu" và "giữ ở đâu" là một câu hỏi bị tách làm đôi:
                      lệnh bán đi theo một tài khoản cụ thể, nên biết tổng mà không
                      biết nó nằm đâu thì vẫn chưa đặt được lệnh.
                    */}
                    <Td>
                      <HoldingAccounts accounts={theoTaiKhoan.get(p.stockId)} />
                    </Td>
                    <Td className="text-right">
            ...
            <table className="w-full min-w-[52rem] text-sm">
              <thead>
                <tr className="border-b border-ink-800 text-left text-xs text-slate-muted">
                  ...
                  <Th>Trạng thái</Th>
                  {/*
                    CỘT DẪN VÀO LỆNH — trang này trước đây là ngõ cụt.

                    Đây là chỗ người ta đứng khi đi sửa lỗi nhập của một thành viên: mở
                    trang của người đó, nhìn thấy đúng dòng sai. Nhưng không dòng nào bấm
                    được, nên phải nhớ mã lệnh rồi sang trang Giao dịch tìm lại — thêm một
                    chỗ để nhầm sang lệnh khác.
                  */}
                  ...
                  return (
                    <tr key={t.id} className="border-b border-ink-800 last:border-0">
                      ...
                      <Td className="text-right">
                        <Link
                          href={`/transactions/${t.id}`}
                          className="text-tiny text-accent-400 hover:text-accent-500"
                          title="Mở chi tiết — sửa khối lượng, đổi chiến lược, huỷ hoặc xoá"
                        >
                          {t.code} →
                        </Link>
                      </Td>
