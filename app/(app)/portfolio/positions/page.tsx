...
export default async function PositionsPage({
  ...
  const [allPositions, theoTaiKhoan, strategies, sectors] = await Promise.all([
    computePositions({ ...applyScope(baseFilter, scope, user.teamId), portfolioId: portfolio.id }),
    // Cùng bộ lọc với dòng trên — xem chú thích trong `computeHoldingsByAccount`.
    computeHoldingsByAccount({
              ...
              <tbody>
                {sorted.map((p) => (
                  ...
                  >
                    ...
                    {/* Giữ ở đâu — xem chú thích cùng cột ở trang cá nhân. */}
                    <td className="px-4 py-2.5">
