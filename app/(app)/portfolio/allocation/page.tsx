...
export default async function AllocationPage() {
  ...
  /* ------------------------------------------------------------------
     §14 Capital Allocation — HAI phần: đang đầu tư và tiền.

     TIỀN LÀ MỘT PHẦN, KHÔNG PHẢI HAI. Trước đây quỹ dự phòng là lát thứ ba;
     người dùng chọn gộp nó vào phần tiền. Lý do đứng vững: cả hai đều là tiền
     mặt đang nằm trong tài khoản — quỹ dự phòng không phải một khoản để chỗ
     khác, nó chỉ là phần được đánh dấu "đừng tiêu". Cắt tiền mặt làm hai lát
     khiến biểu đồ trả lời câu "tiền nằm ở đâu" bằng hai mẩu của cùng một chỗ.

     PHẦN BỊ GIỮ LẠI KHÔNG BIẾN MẤT, nó xuống dòng chú thích ngay dưới. Gộp mà
     không nói ra thì "tiền" ở biểu đồ này và "Tiền khả dụng" ở Dashboard là hai
     con số khác nhau mang cùng một nghĩa trong đầu người đọc — đúng cái nhầm mà
     §14 sinh ra để chặn.
     ------------------------------------------------------------------ */
  const capitalSlices: DonutSlice[] = [
    ...
    },
    {
      key: 'cash',
      label: 'Tiền mặt',
      value: summary.cash.cashBalance,
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card className="p-5">
          ...
          <p className="mt-0.5 mb-4 text-xs text-slate-muted">
            Hai phần cộng lại bằng 100% giá trị danh mục.
          </p>
          ...
          {/*
            QUỸ DỰ PHÒNG NẰM TRONG LÁT "Tiền mặt", nói ra ngay dưới biểu đồ.

            Nó không còn là một lát riêng, nhưng vẫn là số bị trừ khỏi ô KPI "Tiền khả
            dụng". Im lặng ở đây thì hai con số tiền trong cùng một phần mềm lệch nhau
            mà người đọc không có manh mối nào.
          */}
          {summary.cash.reserveAmount > 0n ? (
            <p className="mt-4 border-t border-ink-800 pt-3 text-xs text-slate-muted">
              Trong phần tiền mặt có{' '}
              <Money value={summary.cash.reserveAmount} className="text-warn-500" /> là quỹ dự
              phòng — được giữ lại theo chính sách, nên phần giải ngân được chỉ còn{' '}
              <Money value={summary.cash.deployableCash} className="text-strong" />.
            </p>
          ) : null}
        </Card>
