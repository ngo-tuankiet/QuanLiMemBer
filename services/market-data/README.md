...
Tra tên công ty / sàn / phân ngành ICB của một mã — **không ghi gì vào database**:

```bash
python sync.py info --symbols SCS --out goi-y.json
```

Ai gọi lệnh này: ô "Thêm mã vào danh mục chuẩn" trên trang Market Data, để **gợi ý**
mấy ô còn lại sau khi người dùng gõ mã. Gợi ý, không điền hộ — nguồn xếp ngành sai một
cách rất thuyết phục (VCI xếp SCS, dịch vụ hàng hoá sân bay, vào `5750 Du lịch & Giải
trí` ở ICB cấp 3; phải xuống cấp 4 mới ra `5751 Hàng không`). Quy đổi ICB → ngành của
master data nằm ở `src/market/icb.ts`, và `npm run test:suggest-stock` canh từng dòng
của bảng đó với `sectors`/`industries` thật.

`--out` là **bắt buộc** với `info`, cùng lý do như `groups`: `vnstock` in banner quảng
cáo ra stdout nên "đọc JSON từ stdout" là hợp đồng không đứng vững.
