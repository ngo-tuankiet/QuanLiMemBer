...
async function main(): Promise<void> {
   ...
   */
  /*
   * NHÃN LẤY TỪ TỪ ĐIỂN, không chép tay.
   *
   * Bản trước tìm chuỗi tiếng Anh trong HTML. Giao diện chuyển sang tiếng Việt là cả
   * bài kiểm dừng ở phép đầu và không chạy tới phần nó cần kiểm — đỏ vì một chuỗi nằm
   * trong chính bài kiểm, không vì trang sai.
   */
  const oTV = bocO(member, vi.kpi.portfolioValue);
  const oDM = bocO(pf, vi.kpi.portfolioValue);
  ...
  const NHAN = [
    vi.kpi.portfolioValue,
    vi.kpi.investedCapital,
    vi.kpi.availableCash,
    vi.kpi.totalPnl,
  ];
  ...
  if (!portfolio) {
    ...
    /*
     * SO BẰNG CHUỖI ĐÃ ĐỊNH DẠNG, VÀ ĐỊNH DẠNG BẰNG ĐÚNG HÀM TRANG DÙNG.
     *
     * `MoneyCompact` rút gọn (₫82.4M) nên so số nguyên với HTML sẽ không bao giờ khớp.
     * Nhưng bản trước tự viết lại phép rút gọn, và hai bản KHÔNG giống nhau: hàm thật
     * chia số nguyên nên CẮT phần thừa (164.193.514 → ₫164.193M), bản chép tay chia
     * số thực rồi `toFixed` nên LÀM TRÒN LÊN (→ ₫164.194M). Lệch một chữ số cuối, và
     * bài kiểm đỏ trong khi trang hiển thị hoàn toàn đúng.
     *
     * Gọi thẳng `formatCompactVnd` thì không còn hai bản để lệch nhau nữa.
     */
    const gonLai = formatCompactVnd;

    const oTien = bocO(member, vi.kpi.availableCash);
