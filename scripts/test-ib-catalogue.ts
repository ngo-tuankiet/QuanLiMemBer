...
async function main(): Promise<void> {
  ...
  // -------------------------------------------------------------------------
  const r1 = await saveIbAction(null, fd({ code: 'BUI_HAI', name: 'Bùi Hải', broker: BROKER.SSI, sortOrder: '1' }));
  kiem(r1.ok, 'thêm được IB mới', r1.message ?? '');

  const r2 = await saveIbAction(null, fd({ code: 'BUI_HAI', name: 'Bùi Hải hai', broker: BROKER.SSI, sortOrder: '2' }));
  kiem(!r2.ok, 'mã trùng bị từ chối', r2.fieldErrors?.code?.join(' ') ?? '');

  const r3 = await saveIbAction(null, fd({ code: 'x', name: 'Tên hợp lệ', broker: BROKER.SSI, sortOrder: '0' }));
  kiem(!r3.ok, 'mã sai định dạng bị từ chối', r3.fieldErrors?.code?.join(' ') ?? '');

  await saveIbAction(null, fd({ code: 'NGUYEN_MINH', name: 'Nguyễn Minh', broker: BROKER.SSI, sortOrder: '2' }));
  ...
  /*
   * IB KHÁC CÔNG TY CHỨNG KHOÁN — cũng gửi thẳng, bỏ qua ô chọn đã lọc sẵn.
   *
   * IB ở đây CÓ THẬT và ĐANG BẬT, nên hai chốt cũ đều cho qua. Thứ duy nhất chặn được
   * nó là phép so sàn. Không có ca này thì phép so đó gỡ đi vẫn không bài nào đỏ.
   */
  const ibVps = await prisma.introducingBroker.create({
    data: { code: 'VPS_KHAC_SAN', name: 'IB bên VPS', broker: BROKER.VPS },
    select: { id: true },
  });
  const tkKhacSan = await createBrokerAccountAction(
    null,
    fd({ ...chung, accountNo: 'TEST0005', ibId: ibVps.id }),
  );
  kiem(
    !tkKhacSan.ok,
    'IB khác công ty chứng khoán bị từ chối',
    tkKhacSan.fieldErrors?.ibId?.join(' ') ?? '',
  );
  kiem(
    (await prisma.brokerAccount.count({ where: { accountNo: 'TEST0005' } })) === 0,
    'tài khoản đó KHÔNG được tạo',
  );
  ...
  // -------------------------------------------------------------------------
  await saveIbAction(null, fd({ id: ibBuiHai.id, code: 'BUI_HAI', name: 'Bùi Hải (VPS)', broker: BROKER.SSI, sortOrder: '1' }));
