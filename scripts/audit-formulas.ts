...
/**
 * Những mã mà GIÁ VỐN không thể cộng dồn từ các phần.
 *
 * Giá vốn bình quân gia quyền PHỤ THUỘC TRÌNH TỰ. Khi một mã do nhiều người (hoặc
 * nhiều chiến lược) cùng giao dịch VÀ có lệnh bán, lệnh bán đó rút giá vốn ra theo
 * bình quân của phạm vi đang tính:
 *
 *   toàn danh mục → bình quân của MỌI lệnh mua, gồm cả của người khác
 *   riêng một người → bình quân của chuỗi lệnh CHÍNH NGƯỜI ĐÓ
 *
 * Hai bình quân khác nhau thì phần giá vốn còn lại khác nhau, và Σ các phần KHÔNG
 * bằng tổng. Đây là toán học của phương pháp bình quân, không phải lỗi cộng thiếu.
 *
 * KHỐI LƯỢNG VÀ GIÁ TRỊ THỊ TRƯỜNG KHÔNG BỊ ẢNH HƯỞNG — chúng không phụ thuộc
 * trình tự, nên hai phép kiểm đó vẫn đòi bằng nhau tuyệt đối.
 *
 * Trả về danh sách mã bị ảnh hưởng. Rỗng nghĩa là Σ các phần PHẢI bằng tổng, và
 * lúc đó phép kiểm siết lại thành đẳng thức chính xác.
 */
async function maKhongCongDuocGiaVon(pid: string): Promise<string[]> {
  const rows = await prisma.$queryRaw<{ symbol: string }[]>`
    SELECT s.symbol
    FROM trades t JOIN stocks s ON s.id = t.stockId
    WHERE t.portfolioId = ${pid} AND t.status IN ('EXECUTED', 'PENDING_APPROVAL')
    GROUP BY s.id
    HAVING COUNT(DISTINCT t.userId) > 1
       AND SUM(CASE WHEN t.transactionType = 'SELL' THEN 1 ELSE 0 END) > 0
  `;
...
async function main(): Promise<void> {
  ...
  /*
   * Tính MỘT LẦN ở đây vì hai mục dùng tới nó: chia theo cá nhân (mục G) và chia
   * theo chiến lược. Hai chỗ tự truy vấn riêng thì sớm muộn một chỗ đổi điều kiện
   * và hai mục nói khác nhau về cùng một tập mã.
   */
  const maKhongCong = await maKhongCongDuocGiaVon(pid);
...

  {
     ...
     *   2. tiền = giải ngân được + giữ được   — bất biến, đúng cả khi tiền âm
     *   3. giữ được + còn thiếu = cấu hình     — không đồng nào bốc hơi
     *   4. tiền khả dụng = tiền mặt còn        — KHÔNG trừ quỹ dự phòng
     *
     * Khẳng định 4 là nghĩa MỚI của `availableCash`. Người dùng đọc "tiền khả dụng" là
     * "trong tài khoản còn bao nhiêu", và đó là câu dùng để đối chiếu sao kê. Phần bị
     * giữ lại theo chính sách nay có tên riêng: `deployableCash`.
     */
    ...
    eq(
      cash.deployableCash + cash.reserveAmount,
      cash.cashBalance,
      "Bất biến: tiền = giải ngân được + giữ được",
    );
    /*
     * PHÉP KIỂM CANH ĐÚNG NGHĨA MỚI, và nó phân biệt được: khi quỹ dự phòng khác 0 thì
     * `availableCash` và `deployableCash` BẮT BUỘC khác nhau. Thiếu dòng dưới, một cài
     * đặt trừ lại quỹ dự phòng vào `availableCash` sẽ lọt qua toàn bộ bộ kiểm.
     */
    eq(cash.availableCash, cash.cashBalance, "Tiền khả dụng = tiền mặt còn (không trừ dự phòng)");
    check(
      cash.reserveAmount === 0n || cash.availableCash !== cash.deployableCash,
      'Khi có giữ dự phòng, tiền khả dụng KHÁC phần giải ngân được',
      `khả dụng ${cash.availableCash} · giải ngân ${cash.deployableCash} · giữ ${cash.reserveAmount}`,
    );
    eq(
     ...
     */
    check(
      cash.cashBalance >= 0n ? cash.deployableCash >= 0n : true,
      'Quỹ dự phòng không làm phần giải ngân được âm khi số dư không âm',
...

  {
    ...
    /*
     * QUỸ DỰ PHÒNG NẰM TRONG PHẦN TIỀN, KHÔNG CỘNG THÊM.
     *
     * `cashBps` tính trên TOÀN BỘ số dư tiền kể từ khi biểu đồ phân bổ gộp hai phần
     * làm một. Cộng `reserveBps` vào đây nữa là đếm quỹ dự phòng hai lần — và phép
     * kiểm sẽ vẫn XANH chừng nào quỹ dự phòng còn bằng 0, rồi đỏ đúng vào ngày có
     * người đặt nó lên khác 0. Đó là kiểu phép kiểm tệ nhất: nó ngủ cho tới khi dữ
     * liệu thật xuất hiện.
     */
    const a = summary.allocation;
    const tong = a.investedBps + a.cashBps;
    if (!coDuLieu) {
      skip('Invested + Cash = 100% (§14)', 'danh mục chưa có giao dịch nào — không đánh giá được');
    } else {
      check(
        Math.abs(tong - BPS_TOTAL) <= 3,
        'Invested + Cash = 100% (§14)',
        formatBps(tong, false),
      );
      check(
        a.reserveBps <= a.cashBps,
        'Quỹ dự phòng nằm TRONG phần tiền, không vượt ra ngoài',
        `dự phòng ${formatBps(a.reserveBps, false)} / tiền ${formatBps(a.cashBps, false)}`,
      );
    }
  }
...

  {
    ...
    /*
     * GIÁ VỐN CHỈ CỘNG DỒN ĐƯỢC KHI KHÔNG CÓ MÃ NÀO VỪA CHIA NHIỀU NGƯỜI VỪA CÓ BÁN.
     *
     * Bản trước đòi đẳng thức chính xác. Nó xanh suốt trên dữ liệu demo — nơi mỗi mã
     * chỉ một người giao dịch — rồi đỏ ngay ngày đầu chạy dữ liệu thật, với một con số
     * lệch đúng bằng hiệu hai cách tính bình quân. Một phép kiểm đỏ vì lý do toán học
     * đúng thì tệ hơn không có: nó dạy người ta bỏ qua màu đỏ.
     */
    const tongCaNhan = s((m) => m.investedCost);
    if (maKhongCong.length === 0) {
      eq(tongCaNhan, summary.investedCost, 'Σ giá vốn các cá nhân');
      eq(s((m) => m.totalPnl), summary.totalPnl, 'Σ lãi/lỗ các cá nhân');
    } else {
      /*
       * VẪN PHẢI CANH MỘT ĐIỀU: lệch chỉ được đến từ những mã đó. Nếu chênh lệch lớn
       * hơn hẳn giá vốn của chính chúng thì có thứ khác đang mất tiền, và phép kiểm
       * này phải đỏ. Không có chặn trên thì nó thành lời bào chữa cho mọi sai số.
       */
      const tranLech = summary.positions
        .filter((p) => p.quantity > 0 && maKhongCong.includes(p.symbol))
        .reduce((a, p) => a + p.totalCost, 0n);
      const lech = tongCaNhan - summary.investedCost;
      const abs = lech < 0n ? -lech : lech;
      check(
        abs <= tranLech,
        'Σ giá vốn các cá nhân lệch KHÔNG quá giá vốn của các mã chia nhiều người',
        `lệch ${lech}đ · trần ${tranLech}đ · mã: ${maKhongCong.join(", ")}`,
      );
      skip(
        'Σ giá vốn các cá nhân = tổng (đẳng thức chính xác)',
        `giá vốn bình quân phụ thuộc trình tự — ${maKhongCong.length} mã vừa chia nhiều người vừa có lệnh bán: ${maKhongCong.join(", ")}`,
      );
      skip(
        'Σ lãi/lỗ các cá nhân = tổng (đẳng thức chính xác)',
        'lãi/lỗ tính trên giá vốn, nên thừa hưởng cùng một giới hạn',
      );
    }
...

  {
    ...
    /*
     * Cùng lý do như mục G: một mã bị nhiều chiến lược cùng nắm và có lệnh bán thì
     * giá vốn không cộng dồn được. Nới trần đúng bằng giá vốn của những mã đó, chứ
     * không nới vô hạn — phần còn lại vẫn phải khớp tới từng đồng làm tròn.
     */
    const tranCost =
      BigInt(soLat) +
      summary.positions
    ...
    check(
      ...
      `lệch ${lechCost}đ · trần ${tranCost}đ trên ${soLat} lát`,
    );
  }
