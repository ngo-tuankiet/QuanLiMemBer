...
async function main(): Promise<void> {
  ...
  const ibRieng = await prisma.introducingBroker.create({
    data: { code: 'TEST_RIENG', name: 'IB khai riêng', broker: 'SSI', buyFeeRateBps: 15, sellFeeRateBps: 20 },
  });
  const ibTrong = await prisma.introducingBroker.create({
    data: { code: 'TEST_TRONG', name: 'IB để trống', broker: 'SSI' },
  });
  const ibMienPhi = await prisma.introducingBroker.create({
    data: { code: 'TEST_MIENPHI', name: 'IB miễn phí mua', broker: 'SSI', buyFeeRateBps: 0 },
  });
