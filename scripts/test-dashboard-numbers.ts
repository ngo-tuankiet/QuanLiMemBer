...
async function main(): Promise<void> {
   ...
   */
  /*
   * NHÃN LẤY TỪ CHÍNH TỪ ĐIỂN MÀ TRANG DÙNG, không gõ lại bằng tay.
   *
   * Bản trước tìm chuỗi tiếng Anh "Portfolio Value" trong HTML. Khi giao diện chuyển
   * sang tiếng Việt, bốn phép kiểm này đỏ cùng lúc trong khi trang hoàn toàn đúng — và
   * lý do đỏ nằm ở chỗ khó đoán nhất: một chuỗi chép tay nằm trong bài kiểm.
   *
   * Đọc từ `vi` thì bài kiểm không lệch khỏi giao diện được nữa: sửa chữ trong từ điển
   * là hai bên đổi cùng lúc. Dùng thẳng `vi` chứ không qua `getDict()` vì bài kiểm dựng
   * trang ngoài một request thật nên không có cookie ngôn ngữ — và mặc định là tiếng Việt.
   */
  const oKpi: { nhan: string; giaTri: bigint }[] = [
    { nhan: vi.kpi.portfolioValue, giaTri: portfolioValue },
    { nhan: vi.kpi.investedCapital, giaTri: marketValue },
    { nhan: vi.kpi.availableCash, giaTri: cash.availableCash },
    { nhan: vi.kpi.totalPnl, giaTri: totalPnl },
  ];
